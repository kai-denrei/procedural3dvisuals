...
export const EFFECTS = {
  ...
  'organic-jelly': {
    label: 'Organic Jelly',
    note: 'Translucent specimens — amoeba, adenovirus, jellyfish, bacteriophage — with the melting-jelly refraction look.',
    frag: 'shaders/organic-jelly.frag',
    credit: {
      origin: 'Original to this project',
      via: [
        { label: "translucent shading after noztol's Melting Jello", url: 'https://fragcoord.xyz/s/m78o8vry' },
        { label: 'specimen set referenced from Braille Lab fun-shapes', url: 'https://kai-denrei.github.io/braille-lab/fun-shapes/index.html' },
      ],
      note: 'Shapes and shading written here; standard translucent-raymarch technique, not copied code.',
    },
    params: {
      uSpecies:      I(2, 0, 3, 'Specimen', '0 amoeba · 1 adenovirus · 2 jellyfish · 3 bacteriophage'),
      uPulse:        P(0.5, 0.0, 1.0, 0.005, 'Pulse', 'Life animation. Jellyfish bell contraction, phage sheath firing, adenovirus fibre extension. Only used when Auto pulse is OFF.'),
      uPulseAuto:    P(1.0, 0.0, 1.0, 1.0, 'Auto pulse', 'ON breathes with time. OFF hands it over — drive it from game state.'),
      uSpeed:        P(1.0, 0.05, 4.0, 0.01, 'Speed', 'Animation rate for everything, including the hover bob.'),
      uMelt:         P(0.0, 0.0, 1.0, 0.005, 'Melt', 'Carried over from Melting Jelly: slumps the specimen toward the floor.'),
      uScale:        P(1.0, 0.3, 3.0, 0.01, 'Scale', ''),
      uIOR:          P(1.45, 1.0, 2.2, 0.005, 'Index of refraction', '1.34 is water — which is most of what any of these are.'),
      uAbsorb:       P(0.9, 0.0, 12.0, 0.05, 'Absorption', "Beer's law strength. Higher = denser, more saturated body."),
      uFresnelPow:   P(4.0, 0.5, 12.0, 0.05, 'Fresnel power', 'Rim tightness.'),
      uReflect:      P(0.35, 0.0, 1.0, 0.005, 'Reflectivity', ''),
      uInnerGlow:    P(1.3, 0.0, 3.0, 0.01, 'Inner glow', 'Nucleus / genome / gonads shining through the body. This is what makes it read as alive rather than as a glass ornament.'),
      uSteps:        costly(I(110, 8, 220, 'March steps', '')),
      uRefractSteps: costly(I(28, 2, 60, 'Refraction steps', 'Inside-body march. Cut this first if you need frames.')),
      uStepScale:    P(0.7, 0.2, 1.0, 0.005, 'Step damping', 'Below 1: the amoeba displaces its own surface, so the field is not a true SDF and a full step overshoots.'),
      uFar:          costly(P(22.0, 5.0, 60.0, 0.5, 'Far distance', '')),
      uHue:          P(3.2, 0.0, 6.283, 0.01, 'Hue', ''),
      uHueSpread:    P(1.6, 0.0, 6.283, 0.01, 'Core hue offset', 'Hue of the inner structure relative to the body.'),
      uExposure:     P(0.95, 0.2, 8.0, 0.01, 'Exposure', ''),
      uGrid:         P(1.0, 0.0, 3.0, 0.01, 'Graticule', 'Microscope reticle behind the specimen. Refraction needs something to bend — at 0 the body has nothing to distort and reads as flat.'),
      uGridScale:    P(2.2, 0.3, 12.0, 0.05, 'Graticule scale', ''),
      uSpinY:        P(0.25, -2.0, 2.0, 0.005, 'Spin', ''),
      uCamDist:      P(4.6, 1.5, 15.0, 0.05, 'Camera distance', ''),
      uFov:          P(1.6, 0.5, 4.0, 0.01, 'FOV', ''),
    },
  },
